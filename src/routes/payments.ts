...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const booking = await pool.query(
      'SELECT id, user_id, price, payment_status FROM bookings WHERE id = $1',
      [booking_id]
    );
    const b = booking.rows[0];
    if (!b) return res.status(404).json({ error: 'Booking not found' });
    if (b.user_id !== req.user!.id) return res.status(403).json({ error: 'Forbidden' });
    if (b.payment_status === 'paid') return res.status(400).json({ error: 'Already paid' });
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    await pool.query(
      'INSERT INTO wallet_transactions (wallet_id, amount, type, metadata) VALUES ($1, $2, $3, $4)',
      [wallet.id, -amount, 'withdrawal', JSON.stringify({ requested_at: new Date().toISOString() })]
    );
