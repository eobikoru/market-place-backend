...
router.get('/:id', async (req, res) => {
    const r = await pool.query(`SELECT w.id, w.user_id, w.bio, w.price_min, w.price_max, w.rating, w.review_count, w.verified, w.featured,
    ...
    const reviews = await pool.query(`SELECT r.rating, r.comment, r.created_at, u.name as user_name
     FROM reviews r JOIN users u ON u.id = r.user_id
     WHERE r.worker_id = $1 ORDER BY r.created_at DESC LIMIT 10`, [req.params.id]);
    worker.reviews = reviews.rows;
    return res.json(worker);
});
router.post('/apply', authMiddleware, requireRole('worker'), [
    ...
    try {
        const r = await pool.query(`INSERT INTO workers (user_id, service_category_id, bio, price_min, price_max, id_document_url)
         VALUES ($1, $2, $3, $4, $5, $6)
         ...
         RETURNING id, user_id, service_category_id, bio, price_min, price_max, rating, verified`, [req.user.id, service_category_id, bio || null, price_min, price_max || null, id_document_url || null]);
