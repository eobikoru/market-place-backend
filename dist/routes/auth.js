...
router.post('/login', [body('email').isEmail().normalizeEmail(), body('password').exists()], async (req, res) => {
    ...
    const r = await pool.query('SELECT id, name, email, phone, password_hash, role, avatar_url FROM users WHERE email = $1', [email]);
    const user = r.rows[0];
...
router.get('/me', authMiddleware, async (req, res) => {
    const r = await pool.query(`SELECT id, name, email, phone, role, latitude, longitude, avatar_url, created_at
     FROM users WHERE id = $1`, [req.user.id]);
    const user = r.rows[0];
    if (!user)
        return res.status(404).json({ error: 'User not found' });
    if (user.role === 'worker') {
        const w = await pool.query(`SELECT w.id, w.bio, w.price_min, w.price_max, w.rating, w.review_count, w.verified, w.id_verified, w.featured, sc.name as service_name, sc.slug as service_slug
       FROM workers w
       JOIN service_categories sc ON sc.id = w.service_category_id
       WHERE w.user_id = $1`, [req.user.id]);
        user.workerProfile = w.rows;
    }
    const wallet = await pool.query('SELECT balance FROM wallets WHERE user_id = $1', [req.user.id]);
    user.walletBalance = wallet.rows[0]?.balance ?? 0;
